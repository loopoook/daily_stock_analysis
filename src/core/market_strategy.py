...
CN_BLUEPRINT = MarketStrategyBlueprint(
    region="cn",
    title="A股市场四维复盘策略",
    positioning="聚焦指数趋势、量能结构、情绪温度计与板块轮动，形成次日交易计划。",
    principles=[
        "先看指数方向，再看量能结构，然后看情绪温度，最后看板块持续性。",
        "结论必须映射到仓位、节奏与风险控制动作。",
        "判断使用当日数据与近3日新闻，不臆测未验证信息。",
        "量价配合是趋势真实性的核心验证手段；脱离量能的分析结论可靠性存疑。",
    ],
    dimensions=[
        StrategyDimension(
            name="趋势结构",
            ...
            checkpoints=[
                "上证/深证/创业板是否同向（三指数共振强于单指数独强）",
                "放量上涨或缩量下跌是否成立（量价背离需警惕）",
                "关键支撑阻力是否被突破（收盘有效突破优于盘中突破）",
                "均线多头/空头排列状态（MA5/MA10/MA20斜率与排列方向）",
            ],
        ),
        StrategyDimension(
            name="情绪温度计",
            objective="量化市场情绪热度，识别过热或极度悲观的转折信号。",
            checkpoints=[
                "涨停家数与跌停家数之比（涨停>跌停×3为强势，跌停>涨停×2为恐慌）",
                "涨停家数绝对值（>100家为热市，<20家为冷市）",
                "连板高度（最高连板数反映资金聚集度与风险偏好）",
                "全市场涨跌家数比（7:3以上偏多，3:7以下偏空）",
                "两市成交额与近10日均值对比（放量/缩量程度）",
            ],
        ),
        StrategyDimension(
            name="资金博弈",
            objective="识别短线风险偏好与资金流向结构。",
            checkpoints=[
                "成交额是否扩张（相比前5日均值放大或缩小的幅度）",
                "高位股是否出现分歧（首阴/炸板比例）",
                "板块间资金是否存在明显轮动迹象",
            ],
        ),
        StrategyDimension(
            ...
            checkpoints=[
                "领涨板块是否具备事件催化（政策/业绩/行业消息驱动）",
                "板块内部是否有龙头带动（龙头涨停或明显强于板块）",
                "主线持续性评估（已连续几日活跃，是否出现分歧迹象）",
                "领跌板块是否扩散（单板块还是系统性风险）",
            ],
        ),
    ],
    action_framework=[
        "进攻：指数共振上行 + 成交额放大（>均值20%）+ 涨停>50家 + 主线强化 → 积极加仓。",
        "均衡：指数分化或缩量震荡，涨跌停接近 → 控制仓位在5成以内，等待方向确认。",
        "防守：指数转弱 + 跌停扩散（跌停>涨停） + 领跌板块蔓延 → 优先风控、减仓至3成以下。",
        "极端恐慌（跌停>100家）：此时往往是短线反弹时机，但需等待当日收盘后情绪数据确认再决策。",
    ],
)

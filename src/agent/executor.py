...
7. **筹码健康度（chip_health）** — `get_chip_distribution` 返回的 `chip_health` 字段已由系统预计算（健康/一般/警惕），必须将该值**原样填入** `chip_structure.chip_health`，禁止根据原始数字自行重新推断。
...
{{
    ...
    "dashboard": {{
        ...
        "battle_plan": {{
            "sniper_points": {{
                "ideal_buy": "理想买入价（纯数字，如 25.30）",
                "secondary_buy": "备选买入价（比理想价低一点，如 24.60，若价格跌到这里也可以买）",
                "stop_loss": "止损价（纯数字，如 23.80，跌破这个价就必须卖出）",
                "take_profit": "第一止盈价（纯数字，如 29.50，涨到这里先卖一半锁定利润）",
                "take_profit_2": "第二止盈价（纯数字，比第一止盈高，如 33.00，留底仓等待更大行情）",
                "watch_above": "突破追入价（纯数字，如 27.00，只有价格放量涨过这里才考虑追买，否则等回调）"
            }},
            "position_strategy": {{
                "suggested_position": "建议用多少钱买（如'总资金的3成'或'30%'，要解释为什么这个比例）",
                "entry_plan": "没有持仓的人怎么买：写清楚在哪个价格买、买多少、分几次买（用大白话，如'如果今天回调到25元附近，可以建3成仓位；如果明天继续下跌才买，则等24.6元支撑位'）",
                "risk_control": "如何控制风险：用大白话说明止损逻辑（如'跌破23.8元立刻卖出，不犹豫，最多亏损控制在5%以内'）",
                "add_position_trigger": "什么情况下可以再买一点：用价格和K线描述（如'股价在25元附近止跌企稳、成交量缩小，当天收阳线，可以追加2成仓位'，禁止使用rsi/macd等代码）",
                "exit_trigger": "什么情况下主动减仓离场：用价格和K线描述（如'涨到29元后出现大阴线或成交量异常放大，减仓一半；或持仓超过2周股价没有明显动作，考虑离场等待机会'，禁止使用rsi/macd等代码）"
            }},
            "action_checklist": [
                "✅ 或 ⚠️ 或 ❌ 趋势方向：用一句话描述（如'✅ 趋势向上：三条均线从上到下依次排列，走势健康'，或'⚠️ 趋势不明：均线纠缠在一起，方向待确认'）",
                "✅ 或 ⚠️ 或 ❌ 买入位置：用一句话描述（如'✅ 位置合适：当前价格接近支撑位，没有追高风险'，或'⚠️ 位置偏高：股价已从低点上涨超过10%，建议等回调再买'）",
                "✅ 或 ⚠️ 或 ❌ 成交量：用一句话描述（如'✅ 量能正常：缩量调整，卖盘压力小'，或'⚠️ 量能异常：成交量突然放大3倍以上，需警惕'）",
                "✅ 或 ⚠️ 或 ❌ 动量指标：用一句话描述（如'✅ 动量健康：买入动能强劲，没有超买迹象'，或'⚠️ 已经超买：短期涨太快，可能要回调'）",
                "✅ 或 ⚠️ 或 ❌ 筹码结构：用一句话描述（如'✅ 筹码健康：七成持仓者处于盈利状态，抛压不大'，或'⚠️ 套牢盘多：大部分持仓者亏损，反弹时有抛压'）",
                "✅ 或 ⚠️ 或 ❌ 风险信息：用一句话描述（如'✅ 无明显风险：近期无减持/监管/业绩预警消息'，或'❌ 存在风险：大股东发布减持计划，短期有抛压'）"
            ],
            "invalidation_conditions": [
                "什么情况下放弃这个计划：用大白话写2-3条（如'股价跌破23.8元止损价，说明判断错了，必须离场'）",
                "什么情况下需要重新评估：（如'公司突然发布业绩预亏或大股东减持公告，不管盈亏都要先卖出观望'）"
            ]
        }}
    }},
...
4. **检查清单可视化**：`action_checklist` 每项必须以 ✅/⚠️/❌ 开头，用完整的大白话句子描述（含实际数据），不能只写指标名称或代码
5. **风险优先级**：舆情中的风险点要醒目标出
6. **作战计划完整性**：
   - `take_profit_2` 必须与 `take_profit` 不同，代表更高的波段目标
   - `add_position_trigger` 和 `exit_trigger` 必须用价格和K线行为描述，禁止直接出现 rsi_6、macd_bar、bias_ma5 等技术代码
   - `invalidation_conditions` 至少2条，说明何时本计划判断失效

## 作战计划语言风格要求（重要）

**面向对象**：作战计划的读者是普通投资者，不是专业交易员。必须用能让"从未学过技术分析的人"看懂的语言。

**禁止直接出现的专业代码**（在 battle_plan 所有字段中）：
- ❌ `rsi_6`、`rsi_12`、`macd_bar`、`macd_dif`、`bias_ma5`、`volume_ratio` 等指标变量名
- ❌ "零轴上方金叉"、"MACD柱缩短"、"乖离率"、"量比"等纯技术术语（不加解释时禁止单独使用）
- ❌ "MA5>MA10>MA20多头排列"（直接写指标名而不解释含义）

**正确写法示例**（将技术信息转化为人话）：
- ❌ 错误：`"add_position_trigger": "回踩MA5且rsi_6<55时加仓"`
- ✅ 正确：`"add_position_trigger": "股价回调到约25元的支撑位附近止跌，当天出现小阳线或下影线，成交量比前几天明显缩小，可以追加仓位"`

- ❌ 错误：`"action_checklist": ["✅ RSI: rsi_6=52，健康区间"]`
- ✅ 正确：`"action_checklist": ["✅ 买入动能正常：目前涨势力度适中，没有短期过热的迹象，适合介入"]`

- ❌ 错误：`"exit_trigger": "rsi_6>78且MACD顶背驰时减仓"`
- ✅ 正确：`"exit_trigger": "股价在高位出现单日大阴线（涨幅明显缩水甚至翻绿），或成交量突然放大2-3倍但股价没涨多少，说明有人在高位大量抛货，此时减仓一半"`

- ❌ 错误：`"invalidation_conditions": ["跌破MA20，空头排列确立"]`
- ✅ 正确：`"invalidation_conditions": ["股价跌破23.8元止损价并收盘确认，说明本次判断有误，必须止损离场，等待重新评估"]`

**允许使用的术语**（有广泛认知度）：均线、支撑位、阻力位、止损、止盈、成交量、涨停、跌停、K线、阳线、阴线、仓位
